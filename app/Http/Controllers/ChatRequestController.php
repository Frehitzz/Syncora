...
class ChatRequestController extends Controller
{
    ...
    public function store(Request $request):JsonResponse
    {
        ...
        if($user->id == $receiverId){
            ...
            'You cannot send a chat request to yourself.'],422);
        ...
        ->whereHas('users', function (\Illuminate\Database\Eloquent\Builder $q) use ($receiverId){
        ...
        $existingRequest = ChatRequest::query()->where(function (\Illuminate\Database\Eloquent\Builder $q) use ($user, $receiverId){
            // look a chat reqeust where im the sender
            $q->where('sender_id', $user->id)
            ->where('receiver_id', $receiverId);
        })
        ->orWhere(function (\Illuminate\Database\Eloquent\Builder $q) use ($user, $receiverId){
